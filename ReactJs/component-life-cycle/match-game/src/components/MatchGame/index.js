...
class MatchGame extends Component {
  ...
  changeActiveImage = id => {
    ...
    if (id === activeImageId) {
      ...
      const random = Math.floor(Math.random() * imagesLength)
      this.setState({activeImageId: imagesList[random].id})
    } else {
  ...
  playAgain = () => {
    this.setState({isTimerRunning: true, time: 60, score: 0})
    this.timerID = setInterval(this.timer, 1000)
  }

  renderGameBody = () => {
    const {activeTabId, activeImageId} = this.state
    const imagesListModified = imagesList.filter(
      item => item.category === activeTabId,
    )
    const activeImageIndex = imagesList.findIndex(
      item => item.id === activeImageId,
    )
    const activeImage = imagesList[activeImageIndex].imageUrl
    return (
      <div className="game-card">
        <img src={activeImage} alt="match" className="game-images" />
        ...
        <ul className="game-images-list">
          {imagesListModified.map(item => (
            <GameImage
              key={item.id}
              gameImageDetails={item}
